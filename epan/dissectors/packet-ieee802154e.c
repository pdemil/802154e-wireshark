...
static gboolean ieee802154e_cc24xx = TRUE;
...
dissect_ieee802154e_fcf(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, ieee802154e_packet *packet, guint *offset)
{
    ...
    if (tree) proto_item_append_text(tree, " %s", val_to_str_const(packet->frame_type, ieee802154e_frame_types, "Reserved"));
    if (check_col(pinfo->cinfo, COL_INFO)) col_set_str(pinfo->cinfo, COL_INFO, val_to_str_const(packet->frame_type, ieee802154e_frame_types, "Reserved"));
 ...
 *      The dissect_ieee802154e* functions will set the parameters
 *      in the ieee802154e_packet structure, and pass it to this one
...
dissect_ieee802154e_common(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, guint options)
{
    ...
    ieee802154e_packet      *packet = ep_new(ieee802154e_packet);
    ...
    if(!pinfo->fd->flags.visited){
        ieee_hints = se_new0(ieee802154e_hints_t);
        p_add_proto_data(pinfo->fd, proto_ieee802154e, ieee_hints);
    } else {
        ieee_hints = (ieee802154e_hints_t *)p_get_proto_data(pinfo->fd, proto_ieee802154e);
    }
    ...
    if (packet->security_enable && (packet->version == IEEE802154_VERSION_2006)) {
      ...
      packet->security_level = (ieee802154e_security_level)(security_control & IEEE802154_AUX_SEC_LEVEL_MASK);
      packet->key_id_mode = (ieee802154e_key_id_mode)((security_control & IEEE802154_AUX_KEY_ID_MODE_MASK) >> IEEE802154_AUX_KEY_ID_MODE_SHIFT);
	...
	if(packet->version == IEEE802154_VERSION_2012){
		/* Parse the IE list if present*/
		if(packet->ielist_present){
			dissect_ieee802154e_ielist(tvb, pinfo, ieee802154e_tree, /*packet,*/ &offset);
		}
    ...
    if (packet->security_enable && (packet->version == IEEE802154_VERSION_2003)) {
        ...
        packet->security_level = (ieee802154e_security_level)ieee802154e_sec_suite;
    ...
    TRY {
        if ((packet->frame_type == IEEE802154_FCF_BEACON) ||
            ...
            if ((fcs_ok || !ieee802154e_fcs_ok) && (tvb_reported_length(payload_tvb)>0)) {
                /* Attempt heuristic subdissection. */
                if (!dissector_try_heuristic(ieee802154e_heur_subdissector_list, payload_tvb, pinfo, tree, NULL)) {
        ...
        /* If the packet is a command, try to dissect the payload. */
            ...
            switch (packet->command_id) {
              case IEEE802154_CMD_ASRQ:
                ...
                dissect_ieee802154e_assoc_req(payload_tvb, pinfo, ieee802154e_tree, packet);
                break;
              ...
              case IEEE802154_CMD_ASRSP:
                IEEE802154_CMD_ADDR_CHECK(pinfo, proto_root, packet->command_id,
                    (packet->src_addr_mode == IEEE802154_FCF_ADDR_EXT) &&
                    (packet->dst_addr_mode == IEEE802154_FCF_ADDR_EXT));
                dissect_ieee802154e_assoc_rsp(payload_tvb, pinfo, ieee802154e_tree, packet);
                break;
              ...
              case IEEE802154_CMD_DISAS:
                IEEE802154_CMD_ADDR_CHECK(pinfo, proto_root, packet->command_id,
                    (packet->src_addr_mode == IEEE802154_FCF_ADDR_EXT) &&
                    (packet->dst_addr_mode == IEEE802154_FCF_ADDR_EXT));
                dissect_ieee802154e_disassoc(payload_tvb, pinfo, ieee802154e_tree, packet);
                break;
              ...
              case IEEE802154_CMD_COORD_REAL:
                ...
                dissect_ieee802154e_realign(payload_tvb, pinfo, ieee802154e_tree, packet);
                break;
              ...
              case IEEE802154_CMD_GTS_REQ:
                ...
                dissect_ieee802154e_gtsreq(payload_tvb, pinfo, ieee802154e_tree, packet);
                break;

              default:
                /* Unknown Command */
                call_dissector(data_handle, payload_tvb, pinfo, ieee802154e_tree);
                break;
            } /* switch */
        }
...
dissect_ieee802154e_realign(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, ieee802154e_packet *packet)
{
    ...
    if (tree) proto_tree_add_uint(subtree, hf_ieee802154e_realign_pan, tvb, offset, 2, pan_id);
    ...
    if (tree) proto_tree_add_uint(subtree, hf_ieee802154e_realign_caddr, tvb, offset, 2, coord_addr);
    ...
    if (tree) proto_tree_add_uint(subtree, hf_ieee802154e_realign_channel, tvb, offset, 1, channel);
        ...
        guint16 short_addr, guint16 pan, guint64 long_addr, const char *proto, guint fnum)
{
    ...
    p_map_rec = se_new(ieee802154e_map_rec);
    ...
    if ( g_hash_table_lookup_extended(au_ieee802154e_map->short_table, &addr16, &old_key, NULL) ) {
        ...
        g_hash_table_insert(au_ieee802154e_map->short_table, se_memdup(&addr16, sizeof(addr16)), p_map_rec);
    }
    ...
    if ( g_hash_table_lookup_extended(au_ieee802154e_map->long_table, &long_addr, &old_key, NULL) ) {
        ...
        /* create new hash entry */
        g_hash_table_insert(au_ieee802154e_map->long_table, se_memdup(&long_addr, sizeof(long_addr)), p_map_rec);
    }
...
void proto_register_ieee802154e(void)
{
    ...
    static gint *ett[] = {
        ...
        &ett_ieee802154e_header_ie,
        &ett_ieee802154e_payload_ie,
    };
